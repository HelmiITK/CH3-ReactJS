...
export default function FormAdd() {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        alert(`kamu berhasil menambahkan list: "${name}" ke dalam List Activity kamu`);

        if(!name) return;

    const newItem = { name : name, complete: false, id: Date.now() };
        console.log(newItem)
        ...
        <div className="mt-4 mb-3 p-4 border-2 border-slate-200 rounded-md">
            ...
            <form action="" onSubmit={handleSubmit}>
                <div className="flex">
                    ...
                    <input value={name} onChange={(e) => setName(e.target.value)} className="border rounded-e-md p-2 pl-3 w-full" type="text" placeholder="Input/Edit Todo" id="edit" />
                </div>
